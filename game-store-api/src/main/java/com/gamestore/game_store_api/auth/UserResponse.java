...
public record UserResponse(Long id, String email, Role role, LocalDateTime createdAt) {

	static UserResponse from(UserAccount account) {
		return new UserResponse(account.getId(), account.getEmail(), account.getRole(), account.getCreatedAt());
	}
}
